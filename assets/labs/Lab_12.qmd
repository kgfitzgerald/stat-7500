...
In this lab, you will conduct some exploratory data analysis on the the `gapminder` dataset.
...
Utilize the `group_by` and `summarize()` functions to compute the mean and median life expectancy by continent. Hint: refer to how `group_by()` and `summarize()` were used in Lab 11 to compute the empirical expected value for each simulated sample size.
